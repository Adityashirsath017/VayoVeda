...
import { db, collection, query, where, getDocs } from "../../config/firebaseConfig";
      ...
      const fetchUserAndTasks = async () => {
        try {
          ...
          if (!querySnapshot.empty) {
            ...
            // Check if user is a caretaker (has elders array)
            const hasElders = userData.elders && Array.isArray(userData.elders) && userData.elders.length > 0;
            setIsCaretaker(hasElders);

            // Only fetch today's tasks if user is NOT a caretaker (i.e., user is an elder)
            if (!hasElders && userData.tasks && Array.isArray(userData.tasks)) {
              let allTodayTasks: any[] = [];
              const local = new Date();
              const offset = local.getTimezoneOffset();
              const localDate = new Date(local.getTime() - (offset * 60 * 1000));
              const todayString = localDate.toISOString().split("T")[0];

              const elderName = `${userData.firstName || userData.name || "Me"} ${userData.surname || ""}`.trim();
              const todayTasksList = userData.tasks
                .filter((t: any) => t.date === todayString)
                .map((t: any) => ({
                  ...t,
                  elder: elderName,
                  status: t.status || "NA",
                }));
              allTodayTasks = [...allTodayTasks, ...todayTasksList];
  ...
  return (
    ...
    >
      <>
        ...
        {/* Today's Tasks Section - Only for Elders, NOT for Caretakers */}
        {!isCaretaker && (
